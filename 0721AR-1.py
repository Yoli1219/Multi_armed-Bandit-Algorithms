...
for theta in thetas:
    for epsilon in epsilons:
        ...
        rewards = np.array(
            [generate_ar1_rewards(nodes[node]['mean'], nodes[node]['variance'], T, theta) for node in nodes])
        ...
        # Store results for this epsilon and theta
        key = f"theta={theta}_epsilon={epsilon}"
        results[key] = {
            'actions': actions,
        ...
        print(f"Theta: {theta}, Epsilon: {epsilon}")
        ...
        plt.title(f'Epsilon-Greedy Node Selection Over Time (theta={theta}, epsilon={epsilon})')
        plt.grid(True)
        plt.show()
        ...
        plt.title(f'Cumulative Regret Over Time (theta={theta}, epsilon={epsilon})')
        plt.grid(True)
        plt.show()
        ...
        # plt.title(f'Top-K Picks Over Time (theta={theta}, epsilon={epsilon})')
        # plt.legend()
        # plt.grid(True)
        # plt.show()
        ...
        plt.xlabel('Time')
        plt.ylabel('Regret')
        plt.title(f'Single Trial Regret Over Time (theta={theta}, epsilon={epsilon})')
        plt.grid(True)
        plt.show()

# Plot Top-K accuracy for different epsilons and thetas
for theta in thetas:
    plt.figure(figsize=(12, 8))
    top1_accuracies = [results[f"theta={theta}_epsilon={epsilon}"]['top1_accuracy'] for epsilon in epsilons]
    top2_accuracies = [results[f"theta={theta}_epsilon={epsilon}"]['top2_accuracy'] for epsilon in epsilons]
    top5_accuracies = [results[f"theta={theta}_epsilon={epsilon}"]['top5_accuracy'] for epsilon in epsilons]
    ...
    plt.xlabel('Epsilon')
    plt.ylabel('Accuracy')
    plt.title(f'Top-K Accuracy for Different Epsilons (theta={theta})')
