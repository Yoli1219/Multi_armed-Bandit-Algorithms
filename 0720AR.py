...
alphas = [0.1, 0.5, 0.8]  # List of AR(1) coefficients
...
for theta in alphas:
    for epsilon in epsilons:
        ...
        plt.figure(figsize=(12, 8))
        plt.plot(np.cumsum(results[key]['top1_picks']), label='Top-1 Picks')
        plt.plot(np.cumsum(results[key]['top2_picks']), label='Top-2 Picks')
        plt.plot(np.cumsum(results[key]['top5_picks']), label='Top-5 Picks')
        plt.xlabel('Time')
